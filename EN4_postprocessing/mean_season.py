...
if season == "DJF": # only save once otherwise conflicts arise if writing same file simultantously
    mask_xr.to_netcdf(config.dn_out + 'mask_xr.nc', mode="w")
...
# SAVE mask dataset to file
mask_means.to_netcdf(fn_out, mode="w")
print('done')
...
# SAVE mask dataset to file
mask_stats_model.to_netcdf(fn_out.replace("daily.nc","daily_model.nc"), mode="w")
print('done')