...
figsize = (6.5,5)      # Figure size
sharey = True        # Align y axes
sharex = False       # Align x axes
subplot_padding = .4 # Amount of vertical and horizontal padding between plots
...
label_fontsize = 6               # Fontsize of all labels
...
for var_str in ["Temperature", "Salinity"]:
    if var_str == "Temperature": units = "($^{\circ}$C)"
    ...
    for analysis_str in ["MAE", "BIAS"]:
        ...
        fn_out = "FIGS/regional_{0}_{1}.pdf".format(var_name, run_name)
        ...
        # Make legend
        f.legend(p, legend_str, fontsize = legend_fontsize,
                 bbox_to_anchor=(1.0,0.5))

        # Set Figure title and label axes
        f.suptitle(fig_title, fontsize=title_fontsize,
