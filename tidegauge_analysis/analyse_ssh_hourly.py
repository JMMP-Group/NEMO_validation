import sys
#sys.path.append('/home/users/jelt/dbyrne/code/COAsT')
...
def read_nemo_ssh(fn_nemo_data, fn_nemo_domain, fn_nemo_cfg, chunks):
...
def read_nemo_oneatatime(fn_nemo_data, fn_nemo_domain, fn_nemo_cfg, obs, landmask, 
                         chunks):
    ...
    file=file_list[0]
    nemo = coast.Gridded(file, fn_nemo_domain, config=fn_nemo_cfg).dataset
...
class analyse_ssh_hourly():
    ...
    def __init__(self, fn_nemo_data, fn_nemo_domain, fn_nemo_cfg, fn_obs, fn_out,
                         thresholds = np.arange(0,2,0.1),
                         constit_to_save = ['M2', 'S2', 'K1','O1'], 
                         chunks = {'time_counter':100}):
        
        nemo = read_nemo_ssh(fn_nemo_data, fn_nemo_domain, fn_nemo_cfg, chunks)
        ...
        #                                           fn_nemo_cfg, obs, landmask, chunks)
        ...
        stats = xr.Dataset(coords = dict(
                        ...
                        time_month = ('time_month', ntr_monthly_var.time.data),
                        clim_month = ('clim_month', ntr_clim_var.month.data)),
                   data_vars = dict(
...
class plot_stats_ssh_hourly_single_cfg():
